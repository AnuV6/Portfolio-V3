...
const ActionButtons = ({ noSidebar, onPortfolioClick }: { noSidebar?: boolean; onPortfolioClick: (e: React.MouseEvent<HTMLAnchorElement>) => void }) => (
    <div className={`flex flex-col sm:flex-row flex-wrap gap-4 md:gap-5 ${noSidebar ? 'justify-center' : ''}`}>
        ...
        >
            <i className="fab fa-whatsapp text-lg md:text-xl group-hover:scale-125 transition-transform duration-300" />
            Talk with me
        </a>
        <a
            href="#portfolio"
...
const SliderBanner = ({ noSidebar }: { noSidebar?: boolean }) => {
    ...
    return (
        <div
            className="relative w-full min-h-[450px] lg:min-h-0 lg:h-full overflow-hidden rounded-2xl shadow-2xl shadow-primary/20"
            ...
            <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none p-6 md:p-10 lg:p-12">
                ...
                >
                    <div className={noSidebar ? 'text-center mx-auto max-w-4xl' : ''}>
                        {/* Status Badge */}
                        <div className={`inline-block px-5 py-2.5 mb-5 md:mb-7 rounded-full border-2 border-primary/50 bg-primary/10 text-primary font-mono text-xs md:text-sm tracking-wider shadow-lg shadow-primary/30 backdrop-blur-md hover:shadow-primary/50 transition-all duration-300 ${noSidebar ? 'mx-auto' : ''}`}>
                        ...
                        {/* Main Heading */}
                        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white mb-4 md:mb-5 lg:mb-6 leading-tight drop-shadow-[0_4px_30px_rgba(0,212,204,0.4)]">
                            Building Secure,
                            <br />
                        ...
                        {/* Typewriter Effect */}
                        <div className={`text-gray-200 text-base md:text-lg lg:text-xl font-medium tracking-wide mb-4 md:mb-5 min-h-[28px] md:min-h-[32px] flex items-center ${noSidebar ? 'justify-center' : 'justify-start'}`}>
                            ...
                            <span className="neon-typed-text font-bold text-base md:text-lg lg:text-xl relative">
                                ...
                                <span className="inline-block w-0.5 h-5 md:h-6 bg-primary ml-0.5 animate-pulse"></span>
                            </span>
                        </div>

                        {/* Subtitle */}
                        <p className={`text-sm md:text-base lg:text-lg text-gray-300 mb-6 md:mb-8 ${noSidebar ? 'mx-auto' : ''} max-w-3xl leading-relaxed font-medium`}>
